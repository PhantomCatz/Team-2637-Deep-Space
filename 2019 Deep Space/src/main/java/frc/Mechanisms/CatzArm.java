...
public class CatzArm 
{
    ...
    private static final double EXTENTION_STOP_BUFFER_DISTANCE = 0; //TODO, TBD
    private static final double EXTENTION_STOP_BUFFER_STOP = 0; //TODO, TBD
    ...
    *****************************************************************************/
    ...
    public static void extendArm(double power) 
    {
        ...
        else if(isArmLimitExtendedActivated()&&power>0) 
         {
            armExtensionMtrCtrlA.stopMotor();
         }

        else if(isArmLimitRetractedActivated()&&power<0){
            armExtensionMtrCtrlA.stopMotor();
        }
        
        else if(getArmExtensionEncoderCounts() / ARM_COUNTS_PER_INCHES < 0 + EXTENTION_STOP_BUFFER_DISTANCE&&
                power<0)
        {
            armExtensionMtrCtrlA.set(-EXTENTION_STOP_BUFFER_STOP);
        }
        else if(getArmExtensionEncoderCounts() / ARM_COUNTS_PER_INCHES > 46- EXTENTION_STOP_BUFFER_DISTANCE&&
                 power>0)
        {
            armExtensionMtrCtrlA.set(EXTENTION_STOP_BUFFER_STOP);
        }
    ...
    public static void armExtensionPID(double targetLength, double timeOut)
    {
