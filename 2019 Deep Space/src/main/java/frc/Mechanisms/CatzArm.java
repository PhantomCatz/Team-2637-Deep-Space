...
public class CatzArm 
{
    ...
    private static DoubleSolenoid armExtensionSolenoidBrake;

    private final int ARM_EXTENSION_A_MC_CAN_ID = 20;
    private final int ARM_EXTENSION_B_MC_CAN_ID = 21;

    private final int ARM_EXTENSION_A_BRAKE_PORT = 0;  //TODO, tbd ports
    private final int ARM_EXTENSION_B_BRAKE_PORT = 1;
    ...
    private final int ARM_EXTENSION_ENCODER_A_DIO_PORT = 6;
    private final int ARM_EXTENSION_ENCODER_B_DIO_PORT = 7;
  ...
  //  private static final double MAX_EXTENSION_LIMIT_INCHES = 30 / Math.cos(Math.abs(getPivotAngle()));
    ...
    *****************************************************************************/
    ...
    public CatzArm()
    {
        ...
        armExtensionEnc = new Encoder(ARM_EXTENSION_ENCODER_A_DIO_PORT, ARM_EXTENSION_ENCODER_B_DIO_PORT, false, EncodingType.k4X);
        ...
        armExtensionSolenoidBrake = new DoubleSolenoid(ARM_EXTENSION_A_BRAKE_PORT, ARM_EXTENSION_B_BRAKE_PORT);
    ...
    public void extendArm(double power) 
    {
        ...
        if(power != 0)
        {
            releaseArmExtensionBrakes();
            Timer.delay(0.01);
            armExtensionMtrCtrlA.set(power);
        }
        else
        {
            engageArmExtensionBrakes();
        }
    ...
    */
    ...
    public static void engageArmExtensionBrakes()
    {
        armExtensionSolenoidBrake.set(Value.kForward);
    }

    public static void releaseArmExtensionBrakes()
    {
        armExtensionSolenoidBrake.set(Value.kReverse);
    }
